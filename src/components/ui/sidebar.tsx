...
export default function Sidebar ({ className, menuItems, user, onLogout }: SidebarProps): JSX.Element {
  ...
  return (
    <>
      ...
      <div className={cn('transition-[width]', mobileClasses, !sidebarOpen && isMobile && 'pointer-events-none', !sidebarOpen ? 'w-0' : 'w-full')} style={{ maxWidth: 270 }}>
        <ScrollArea className={cn('h-screen bg-background overflow-y-auto border-r relative transition -left-px', className, isMobile && !sidebarOpen && ' -translate-x-full')}>
