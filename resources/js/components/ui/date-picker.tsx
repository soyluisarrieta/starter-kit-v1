...
const MONTHS = [
  { name: 'ene', full: 'enero' },
  { name: 'feb', full: 'febrero' },
  { name: 'mar', full: 'marzo' },
  { name: 'abr', full: 'abril' },
  { name: 'may', full: 'mayo' },
  { name: 'jun', full: 'junio' },
  { name: 'jul', full: 'julio' },
  { name: 'ago', full: 'agosto' },
  { name: 'sep', full: 'septiembre' },
  { name: 'oct', full: 'octubre' },
  { name: 'nov', full: 'noviembre' },
  { name: 'dic', full: 'diciembre' }
]
...
function DatePickerTrigger ({
  ...
  const formatDisplayText = () => {
    const placeholderText = placeholder ? placeholder : `Selecciona ${isRangeMode ? 'un rango de fechas' : 'una fecha'}`
    if (!isRangeMode) {
      return selectedDate ? format(selectedDate ? selectedDate : new Date(), "MMMM d 'de' yyyy") : placeholderText
    } else if (isRangeMode) {
      ...
      {calendarDays.map((day, i) => (
        <div key={i}>
          {day ? (
            <Button
              variant="ghost"
              title={isToday(day) ? 'Hoy' : undefined}
              className={cn(
                // Base style
          ...
          {onReset && ((!isRangeMode && selectedDate) || (isRangeMode && (rangeStart || rangeEnd))) && (
            <div className='border-t'>
              <Button className="w-full mt-2" variant="secondary" onClick={handleReset}>
                Limpiar {isRangeMode ? 'rango' : 'fecha'}
              </Button>
            </div>
          )}
        </div>
